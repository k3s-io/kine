...
func setupBackend(ctx context.Context, t *testing.T) (*server.Server, *nats.Conn, *Backend) {
	...
	ekv := NewKeyValue("local", bkt, js, 10, b.Delete)
...
func TestBackend_List(t *testing.T) {
	...
	// Create a key.
	b.Create(ctx, prefix("/a/b/c"), nil, 0)
	b.Create(ctx, prefix("/a"), nil, 0)
	b.Create(ctx, prefix("/b"), nil, 0)
	b.Create(ctx, prefix("/a/b"), nil, 0)
	b.Create(ctx, prefix("/c"), nil, 0)
	b.Create(ctx, prefix("/d/a"), nil, 0)
	b.Create(ctx, prefix("/d/b"), nil, 0)
...
func TestBackend_Watch(t *testing.T) {
	...
	b.Delete(ctx, prefix("/a"), rev1)
	b.Update(ctx, prefix("/a/1"), nil, rev2, 0)

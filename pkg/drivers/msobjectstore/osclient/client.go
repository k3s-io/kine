...
func (s *store) Store(cmd types.StoreCmd) (err error) {
	...
	logrus.Infof("msobjectstore Client.Store JSON: %s", string(prettyJSON.Bytes()))
...
func (s *store) GetValue(cmd types.GetCmd) (res types.GetResult, err error) {
	...
	defer func() {
		dur := time.Since(start)
		fmt := "msobjectstore Client.GetValue %s store=%s, partition=%s => key=%s, version=%s, err=%v, duration=%s"
		resKey := ""
		resVer := ""
		if res != nil {
		...
		s.logMethod(dur, fmt, cmd.GetKey(), cmd.GetStore(), cmd.GetPartition(), resKey, resVer, err, dur)
	}()
